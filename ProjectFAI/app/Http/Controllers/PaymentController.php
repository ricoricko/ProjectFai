    ...
    public function paymentSuccess(Request $request)
{
    ...
    foreach ($cartItems as $item) {
        ...
        if (!$stok) {
            return redirect()->route('cart.view')->with('error', 'Stok tidak ditemukan untuk item: ' . $item->nama_menu);
        }
        ...
        $totalCashIn += $item->jumlah * $item->harga_menu;
    }
    ...
    $currentCash = DB::table('cash')->orderBy('id_cash', 'desc')->first();

    if ($currentCash) {
        DB::table('cash')->where('id_cash', $currentCash->id_cash)->update([
            'jumlah_cash' => $currentCash->jumlah_cash + $totalCashIn,
            'tanggal' => now(),
        ]);
    } else {
        DB::table('cash')->insert([
            'jumlah_cash' => $totalCashIn,
            'tanggal' => now(),
        ]);
    }
